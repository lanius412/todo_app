...
func (u *User) CreateSession() (session Session, err error) {
	session = Session{}
	cmdC := `insert into sessions (
		uuid,
		email,
		user_id,
		created_at) values ($1, $2, $3, $4)`
	_, err = Db.Exec(cmdC, createUUID(), u.Email, u.ID, time.Now())
	if err != nil {
		log.Fatalln(err)
	}
	cmdG := `select id, uuid, email, user_id, created_at from sessions where user_id = $1 and email = $2`
	err = Db.QueryRow(cmdG, u.ID, u.Email).Scan(
		&session.ID,
		&session.UUID,
		&session.Email,
		&session.UserID,
